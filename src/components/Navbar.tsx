  ...
  const UserMenu = () => (
    <div className="relative" ref={userMenuRef}>
      <button
        ...
        className={`flex items-center space-x-2 px-3 py-2 text-sm font-medium transition-all duration-300 group ${
          scrolled 
        ...
        <div className="flex justify-between h-20">
          ...
          <div className="flex items-center">
            <Link href="/" className="flex items-center space-x-3 group">
              ...
              <span className={`text-2xl font-bold group-hover:text-green-600 transition-colors duration-300 ${
            ...
            {navigation.map((item) => (
              ...
              >
                ...
                <span className={`absolute bottom-0 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full ${
                  scrolled ? 'bg-green-400' : 'bg-green-600'
                }`}></span>
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center space-x-4">
            {status === 'loading' ? (
              <div className={`animate-spin rounded-full h-6 w-6 border-b-2 ${
                scrolled ? 'border-green-400' : 'border-green-600'
              }`}></div>
            ) : session ? (
              <UserMenu />
            ) : (
            ...
            <button
              ...
              className={`md:hidden transition-colors duration-300 ${
                scrolled ? 'text-white hover:text-green-400' : 'text-gray-700 hover:text-green-600'
              }`}
            >
              {isOpen ? (
